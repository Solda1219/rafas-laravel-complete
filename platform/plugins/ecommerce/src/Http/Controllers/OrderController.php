...
class OrderController extends BaseController
{
    ...
    public function postCancelOrder($id, BaseHttpResponse $response)
    {
        ...
        //find user id by order id;
        $customerId= $this->userIdByorderId($id);
        //find fcm user token by user_id;
        $device_token= $this->fcmUserTokenByUserId($customerId);
        $title= 'Order Status changed!';
        $content= OrderStatusEnum::CANCELED;
        $orderId= $id;
        $this->sendNotification($device_token, $title, $content, $orderId);
    ...
    //this is part for fcm notification to React Native
    public function sendNotification($firebaseToken, $title, $content, $orderId)
    {
        // $firebaseToken = ["c_NraPz0QpSBCJJ_mhJFya:APA91bEuW_tJhiEu74GVZbnwPFfCiE_JB1rgR6G6LpyjowOhJNde2_za3zDOpt_2k9mSIrKKxHsWMS4aCR_HDxe-ZCK89yQCm8GVZf1210rwAo392-oy2-D4JuJ_-LZSt5kPiPP_ywxQ"];
        ...
        $SERVER_API_KEY = "AAAAulKTTQg:APA91bG7P1e_BIdCy97Xl9ssqGNAWkDcN00towxMN9K8nCJ-ohw2DJvz3T4hRqtnUwfE9ed6or-slmKm5GbfHySOhC_gOzvVwmQ0KFMnn43gRqtqB8rI_-a_74QFofoNhdWh_rC3WOCr";
        $body= "Your order #".$orderId." changed to ".$content.".";
        $data = [
    ...
    public function fcmUserTokenByUserId($id){
        $fcmUsers= DB::select('select * from ec_customers_token where user_id= ?', [$id]);
        $device_token= [];
        if(count($fcmUsers)>0){
    ...
    public function userIdByorderId($id){
        $payments= DB::select('select * from payments where order_id= ?', [$id]);
        $customerId= 0;
        if(count($payments)>0){
    ...
    public function postConfirmPayment($id, BaseHttpResponse $response)
    {
        ...
        $title= 'Order Status changed!';
        $content= PaymentStatusEnum::COMPLETED;
        $orderId= $id;
        $this->sendNotification($device_token, $title, $content, $orderId);
    ...
    public function postRefund($id, RefundRequest $request, BaseHttpResponse $response)
    {
        ...
        $device_token= $this->fcmUserTokenByUserId($customerId);
        $title= 'Order Status changed!';
        $content= PaymentStatusEnum::REFUNDED;
        $orderId= $id;
        $this->sendNotification($device_token, $title, $content, $orderId);
